...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        ...
        if (regFuture.isDone()) { //执行完
            // At this point we know that the registration was complete and successful.
            ChannelPromise promise = channel.newPromise();
            doBind0(regFuture, channel, localAddress, promise);
            return promise;
        } else { //未执行完,添加监听事件,等待执行完成执行doBind0()
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            ...
            // as the Channel is not registered yet we need to force the usage of the GlobalEventExecutor
            //返回带异常的 DefaultChannelPromise 对象。因为创建 Channel 对象失败，所以需要创建一个 FailedChannel 对象，
            // 设置到 DefaultChannelPromise 中才可以返回。
        ...
        if (regFuture.cause() != null) {
            if (channel.isRegistered()) {
                ...
                //若发生异常，并且 Channel 并未注册成功，则调用 #closeForcibly() 方法，强制关闭
